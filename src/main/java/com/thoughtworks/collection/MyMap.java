...
public class MyMap {
    ...
    public List<String> mapLetter() {
        ...
        List<Integer> byteNumber=this.array.stream().map(item->item+96).collect(Collectors.toList());

        return byteNumber.stream().map(item->Character.toString((char)item.intValue())).collect(Collectors.toList());
    }
